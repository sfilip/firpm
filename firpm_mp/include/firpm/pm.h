...
typedef std::pair<mpfr::mpreal, mpfr::mpreal> interval_t;
...
struct pmoutput_t
{
    std::vector<mpfr::mpreal> h;    /**< the final filter coefficients*/
    std::vector<mpfr::mpreal> x;    /**< the reference set used to
                                      generate the final filter (values are in \f$[-1,1]\f$
                                      and NOT \f$[0,\pi]\f$)*/
    std::size_t iter;               /**< number of iterations that were necessary to achieve
                                      convergence*/
    mpfr::mpreal delta;             /**< the final reference error */
    mpfr::mpreal q;                 /**< convergence parameter value */
};
 ...
 * @param prec MPFR working precision used to perform the computations
 */
void referenceScaling(std::vector<mpfr::mpreal>& nx, std::vector<band_t>& ncbands,
        std::vector<band_t>& nfbands, std::size_t nxs,
        std::vector<mpfr::mpreal>& x, std::vector<band_t>& cbands,
        std::vector<band_t>& fbands, mp_prec_t prec = 165ul);

/*! An internal routine which implements the exchange algorithm for designing
 * FIR filters
 * @param[in] x the initial reference set
 * @param[in] chebyBands band information for the filter to which the x reference
 * corresponds to. The bands are given inside \f$[-1,1]\f$ (i.e., the CHEBY band space)
 * @param[in] epsT convergence parameter threshold (i.e quantizes the number of 
 ...
 * the h vector of the output will correspond o the coefficients of the frequency'
 ...
 * fbands[0].weight = [] (space_t, mpfr::mpreal) -> mpfr::mpreal {return mpfr::mpreal(1); };
 * fbands[0].space = space_t::FREQ;
 * fbands[0].amplitude = [](space_t, mpfr::mpreal) -> mpfr::mpreal { return mpfr::mpreal(1); };
 *
 * fbands[1].start = pi * 0.5;
 * fbands[1].stop = pi;
 * fbands[1].weight = [] (space_t, mpfr::mpreal) -> mpfr::mpreal {return mpfr::mpreal(10); };
 * fbands[1].space = space_t::FREQ;
 * fbands[1].amplitude = [](space_t, mpfr::mpreal) -> mpfr::mpreal { return mpfr::mpreal(0); };
 ...
 * uniform(omega, fbands, prec);
 * // apply the change of variable y = cos(x) so that we are working inside [-1, 1]
 * cos(x, omega);
 * bandConversion(cbands, fbands, convdir_t::FROMFREQ);
 ...
 * inside the stopband. More examples, including code on how to use the reference scaling 
 * and AFP versions <tt>firpmRS, firpmAFP</tt>, are provided inside the test files.
 * @code
