...
struct band_t {
    ...
    std::function<mpfr::mpreal(space_t, mpfr::mpreal)> amplitude;
    ...
    std::function<mpfr::mpreal(space_t, mpfr::mpreal)> weight;
 ...
 * @param[in]  prec the working precision used to perform 
 * the computations
 */
void bandconv(std::vector<band_t> &out, std::vector<band_t> &in,
        convdir_t direction, mp_prec_t prec = 165ul);
