...
void baryweights(std::vector<long double>& w,
...
void idealvals(long double &D, long double &W,
        const long double &xVal, std::vector<band_t> &bands)
{
    for (auto &it : bands) {
...
void compdelta(long double &delta, std::vector<long double>& x,
...
void compdelta(long double &delta, std::vector<long double>& w,
        std::vector<long double>& x, std::vector<band_t> &bands)
{
    long double num, denom, D, W, buffer;
    num = denom = D = W = 0;
    for (std::size_t i{0u}; i < w.size(); ++i)
...
void compc(std::vector<long double> &C, long double &delta,
        std::vector<long double> &omega, std::vector<band_t> &bands)
{
    long double D, W;
    D = W = 0;
...
void approx(long double &Pc, const long double &omega,
...
void comperror(long double &error, const long double &xVal,
