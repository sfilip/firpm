...
void chebvand(MatrixXld& A, std::size_t degree, 
        std::vector<long double>& meshPoints,
        std::function<long double(long double)>& weightFunction)
{

    A.resize(degree + 1u, meshPoints.size());
    for(std::size_t i{0u}; i < meshPoints.size(); ++i)
    {
        long double pointWeight = weightFunction(meshPoints[i]);
        A(0u, i) = 1;
        A(1u, i) = meshPoints[i];
...
void afp(std::vector<long double>& points, MatrixXld& A, 
         std::vector<long double>& mesh)
{
    VectorXld b = VectorXld::Ones(A.rows());
    b(0) = 2;
    VectorXld y = A.colPivHouseholderQr().solve(b);
    points.clear();
...
void countBand(std::vector<band_t>& cb, std::vector<long double>& x)
{
    for(auto& it : cb)
        it.xs = 0u;
...
void wam(std::vector<long double>& wam, std::vector<band_t>& cb, 
         std::size_t deg)
{
    std::vector<long double> cp;
    equipts(cp, deg + 2u);
    cos(cp, cp);
    ...
    for(std::size_t i{0u}; i < cb.size(); ++i)
    {
        ...
        {
            std::vector<long double> bufferNodes;
...
void uniform(std::vector<long double>& omega,
        std::vector<band_t>& B, std::size_t n)
{
    long double avgDist{0.0l};
    omega.resize(n);
...
void referenceScaling(std::vector<long double>& newX, 
        std::vector<band_t>& newChebyBands, std::vector<band_t>& newFreqBands,
        std::size_t newXSize, std::vector<long double>& x, 
        std::vector<band_t>& chebyBands, std::vector<band_t>& freqBands)
{
        ...
        std::vector<long double> &x)
{
    ...
    for(std::size_t i{0u}; i < chebyBands.size(); ++i)
    {
        long double middleValA{0.0l}, middleValB{0.0l};
...
void extremaSearch(long double& convergenceOrder,
        long double& delta, std::vector<long double>& eigenExtrema,
        std::vector<long double>& x, std::vector<band_t>& chebyBands,
        std::size_t Nmax)
{
    ...
    interval_t dom{std::make_pair(-1.0l, 1.0l)};
    ...
    for (std::size_t i = 0u; i < subIntervals.size(); ++i)
    {
        ...
        // derivative
        std::vector<long double> c(Nmax + 1u);
        chebcoeffs(c, fx);
        std::vector<long double> dc(Nmax);
        diffcoeffs(dc, c);
    ...
    else if (alternatingExtrema.size() > x.size())
    {
        ...
        while (alternatingExtrema.size() > x.size())
        {
            ...
            long double minValToRemove = fminl(fabsl(alternatingExtrema[0u].second),
...
pmoutput_t exchange(std::vector<long double>& x,
        std::vector<band_t>& chebyBands, long double eps,
        std::size_t Nmax)
{
    pmoutput_t output;
    ...
    equipts(finalChebyNodes, degree);
...
void parseSpecification(std::vector<long double> const &f,
...
pmoutput_t firpm(std::size_t n, 
            ...
            std::vector<long double>const &a,
            std::vector<long double>const &w,
            long double eps, std::size_t nmax,
            init_t strategy, 
            std::size_t depth,
            init_t rstrategy)
{
    ...
    if(n % 2 == 0) {            // type I filter
        for(std::size_t i{0u}; i < fbands.size(); ++i) {
            ...
            fbands[i].weight = [i, &w](space_t, long double x) -> long double {
                return w[i];
            };
        }
    } else {                    // type II filter
        for(std::size_t i{0u}; i < fbands.size(); ++i) {
            ...
            fbands[i].weight = [i, &w](space_t space, long double x) -> long double {
                if(space == space_t::FREQ)
                    return cosl(x/2) * w[i];
                else
    ...
    std::function<long double(long double)> wf = [&cbands](long double x) -> long double {
    ...
    switch(strategy) {
        ...
        case init_t::SCALING: 
        {
            ...
            for(std::size_t i{1u}; i <= depth && output.q <= 0.5l; ++i) {
                x.clear();
        ...
        std::size_t depth, init_t rstrategy)
{
    ...
    std::size_t deg = n / 2u;
    long double sFactor = a[1] / (f[1] * M_PI);
    ...
    if(n % 2 == 0) { // TYPE III
        ...
        if(f[f.size() - 1u] == 1.0l) {
            if(f[f.size() - 2u] > 0.9999l)
                fn[f.size() - 1u] = (1.0l+ f[f.size() - 2u]) / 2;
            else
        ...
        if(type == filter_t::FIR_DIFFERENTIATOR) {
            fbands[0u].weight = [&w](space_t space, long double x) -> long double {
                if(space == space_t::FREQ)
                    return (sinl(x) / x) * w[0u];
                else
                    return (sqrtl(1.0l - x * x) / acosl(x)) * w[0u];                    
            };
            fbands[0u].amplitude = [sFactor](space_t space, long double x) -> long double {
                if(space == space_t::FREQ)
            ...
            fbands[0u].amplitude = [&a, &fbands](space_t space, long double x) -> long double {
                if(space == space_t::CHEBY)
                    x = acosl(x);
                if(a[0u] != a[1u]) 
        ...
        for(std::size_t i{1u}; i < fbands.size(); ++i) {
            ...
            fbands[i].space = space_t::FREQ;
            if(type == filter_t::FIR_DIFFERENTIATOR) {
                fbands[i].weight = [&w, i](space_t space, long double x) -> long double {
                    if(space == space_t::FREQ)
                        return sinl(x) * w[i];
                    else
                        return sqrtl(1.0l - x * x) * w[i];
                };
                fbands[i].amplitude = [&fbands, &a, i](space_t space, long double x) -> long double {
                    if(a[2u * i] != a[2u * i + 1u]) {
                        if(space == space_t::CHEBY) {
                            x = acosl(x);
                            return ((x - fbands[i].start) * a[2u * i + 1u] -
        ...
        if(type == filter_t::FIR_DIFFERENTIATOR) {
            ...
            fbands[0u].amplitude = [&fbands, &a](space_t space, long double x) -> long double {
                if(space == space_t::CHEBY)
                    x = acosl(x);
                if(a[0u] != a[1u]) 
                    return (((x - fbands[0].start) * a[1u] -
