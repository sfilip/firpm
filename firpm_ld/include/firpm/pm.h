...
typedef std::pair<long double, long double> interval_t;
...
struct pmoutput_t
{
    std::vector<long double> h;    /**< the final filter coefficients*/
    std::vector<long double> x;    /**< the reference set used to generate the final 
                                filter (values are in \f$[-1,1]\f$ and NOT \f$[0,\pi]\f$)*/
    std::size_t iter;         /**< number of iterations that were necessary to 
                                achieve convergence*/
    long double delta;             /**< the final reference error */
    long double q;                 /**< convergence parameter value */
};
 ...
 * band space)
 */
void referenceScaling(std::vector<long double>& nx, std::vector<band_t>& ncbands,
        std::vector<band_t>& nfbands, std::size_t nxs,
        std::vector<long double>& x, std::vector<band_t>& cbands,
 ...
 * fbands[0].weight = [] (space_t, long double) -> long double {return 1.0; };
 * fbands[0].space = space_t::FREQ;
 * fbands[0].amplitude = [](space_t, long double) -> long double { return 1.0; };
 *
 * fbands[1].start = pi * 0.5;
 * fbands[1].stop = pi;
 * fbands[1].weight = [] (space_t, long double) -> long double {return 10.0; };
 * fbands[1].space = BandSpace::FREQ;
 * fbands[1].amplitude = [](space_t, long double) -> long double { return 10.0; };
