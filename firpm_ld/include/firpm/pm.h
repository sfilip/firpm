...
typedef std::pair<long double, long double> Interval;
...
struct PMOutput
{
    ...
    std::vector<long double> x;    /**< the reference set used to
 ...
 * freqBands[0].weight = [] (BandSpace, long double) -> long double {return long double(1); };
 * freqBands[0].space = BandSpace::FREQ;
 * freqBands[0].amplitude = [](BandSpace, long double) -> long double { return long double(1); };
 *
 * freqBands[1].start = pi * 0.5;
 * freqBands[1].stop = pi;
 * freqBands[1].weight = [] (BandSpace, long double) -> long double {return long double(10); };
 * freqBands[1].space = BandSpace::FREQ;
 * freqBands[1].amplitude = [](BandSpace, long double) -> long double { return long double(0); };
